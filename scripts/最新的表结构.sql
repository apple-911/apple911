...
-- public.sys_code_types 定义

-- Drop table

-- DROP TABLE public.sys_code_types;

CREATE TABLE public.sys_code_types (
	id text NOT NULL,
	"name" text NOT NULL,
	description text NULL,
	...
	created_at timestamptz DEFAULT now() NULL,
	updated_at timestamptz DEFAULT now() NULL,
	CONSTRAINT sys_code_types_pkey PRIMARY KEY (id)
);

-- Table Triggers

create trigger trigger_sys_code_types_updated_at before
update
    on
    public.sys_code_types for each row execute function update_updated_at();
...
-- DROP TABLE public.sys_codes;

CREATE TABLE public.sys_codes (
	id serial4 NOT NULL,
	...
	created_at timestamptz DEFAULT now() NULL,
	updated_at timestamptz DEFAULT now() NULL,
	CONSTRAINT sys_codes_pkey PRIMARY KEY (id),
	CONSTRAINT sys_codes_type_id_code_key UNIQUE (type_id, code),
	CONSTRAINT sys_codes_type_id_fkey FOREIGN KEY (type_id) REFERENCES public.sys_code_types(id)
);
CREATE INDEX idx_sys_codes_code ON public.sys_codes USING btree (code);
CREATE INDEX idx_sys_codes_type_id ON public.sys_codes USING btree (type_id);

-- Table Triggers

create trigger trigger_sys_codes_updated_at before
update
    on
    public.sys_codes for each row execute function update_updated_at();
...
CREATE TABLE public.audit_history (
	...
	created_at timestamptz DEFAULT now() NULL,
	"comment" text NULL,
	operator_type text DEFAULT 'pending'::text NULL,
...
CREATE INDEX idx_audit_history_operator_type ON public.audit_history USING btree (operator_type);
