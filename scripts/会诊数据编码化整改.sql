...
('consultation_type', 'inhospital', '院内会诊', '在医院内部进行的会诊', 'blue', 1),
('consultation_type', 'remote', '远程会诊', '通过远程方式进行的会诊', 'green', 2)
...
-- 6. 更新流程节点码表 - 添加重新排期节点（与数据库现有数据一致）
INSERT INTO sys_codes (type_id, code, name, description, color, sort_order) VALUES
...
SET urgency = CASE 
  ...
  WHEN urgency = '特急' OR urgency = '较急' THEN 'critical'
  ELSE urgency
END
...
UPDATE audit_history 
SET node = CASE 
  WHEN node = '申请提交' THEN 'apply_submit'
  ...
  WHEN node = '专家确认' THEN 'expert_confirm'
  WHEN node = '会议安排' THEN 'meeting_schedule'
  WHEN node = '会议记录' THEN 'meeting_record'
  WHEN node = '报告提交' THEN 'report_submit'
  WHEN node = '撤回' THEN 'revoke'
  ELSE node
END
WHERE node NOT IN (
  'apply_submit', 'department_audit', 'secretary_audit', 'rescheduled',
  'expert_confirm', 'meeting_schedule', 'meeting_record', 'report_submit', 'revoke'
);
...
-- 验证码表数据
SELECT '会诊类型码表' as 码表，code, name, color FROM sys_codes WHERE type_id = 'consultation_type' ORDER BY sort_order;
SELECT '紧急程度码表' as 码表，code, name, color FROM sys_codes WHERE type_id = 'urgency_level' ORDER BY sort_order;
SELECT '审核结果码表' as 码表，code, name, color FROM sys_codes WHERE type_id = 'audit_result' ORDER BY sort_order;
